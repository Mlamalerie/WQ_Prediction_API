...
def test_can_get_serialized_model():
    ...
    response = requests.get(ENDPOINT + f"/api/model?model_name={ModelName.linear}")
    assert response.status_code == 200
    assert response.headers["Content-Type"] == "application/octet-stream"
...
def test_can_add_wine():
    ...
    add_wine_response = requests.put(ENDPOINT + "/api/model", json=wineq.dict())
    ...
    wine = Wine(fixed_acidity=7.4, volatile_acidity=0.7, citric_acid=0, residual_sugar=1.9, chlorides=0.076,
                          ...
                          alcohol=9.4)
    payload = wine.dict()
    add_wine_response = requests.put(ENDPOINT + "/api/model", json=wine.dict())
    assert not add_wine_response.status_code == 201
